...
leaderboard (gross, net, Stableford) across every round. Runs great on GitHub Pages,
with an optional Google Sheet backend so everyone's scores sync automatically.

## Live Sync — recommended setup

By default, the site saves to whoever's browser is entering data, and you'd have to
manually export/commit `data.json` for others to see it. **Live Sync fixes this**: it
connects the site to a Google Sheet so any scorecard, roster change, or round update
from any device shows up for everyone automatically, usually within seconds.

Setup takes about 5–10 minutes and involves no coding — see
**`google-sheet-backend/SETUP_GOOGLE_SHEET.md`** for the full walkthrough. Once
connected, a small badge near the top of every page shows sync status ("Synced Xs
ago"), and there's a ↻ refresh button if you don't want to wait for the automatic
20-second check.

If you skip this setup, the site still works exactly as before — see "Backup /
Restore (manual, no Live Sync)" below for the export/commit flow.
...
**With Live Sync connected** (see above), it's simple:
...
2. **During a match-play round**: open that round's tab on any phone. Each foursome
   has its own scorecard — enter all 4 players' 18 holes (front 9 / back 9 / total
   add up live), hit save. The match status updates immediately, calculated straight
   from best-ball (or head-to-head for singles) on each hole. No login needed.
3. **During a scramble/skins round**: there's no foursome breakdown — one player
   ...
   someone sets the round's winner directly.
4. **Contests**: the Closest to the Pin dropdown lives at the top of each round page.
5. That's it — every save goes straight to the shared sheet, and everyone else's
   page picks it up within about 20 seconds (or instantly with the ↻ refresh button).

**Without Live Sync connected**, saves only live in the browser doing the entering,
so someone needs to periodically publish an update for everyone else to see it — see
"Backup / Restore (manual, no Live Sync)" below for that flow.

## Editing the roster before the trip

Go to Admin → Players to rename the 12 placeholder players, set handicaps, and assign
each to a team. Do this once at the start; with Live Sync connected it's visible to
everyone immediately, otherwise export/commit so everyone loads the real roster.

## Backup / Restore (manual, no Live Sync)

If you haven't set up Live Sync, this is how updates get published — and even with
Live Sync connected, it's worth knowing as a manual backup option:

1. In Admin → **Backup / Restore**, click **Export data.json**. It downloads the
   current state as a file.
2. Replace `data/data.json` in the repo with that file (easiest: on github.com, open
...
3. If you're entering scores from a different device/browser than before (and not
   using Live Sync), use **Import JSON** first to load the latest `data.json` so
   you're not overwriting anyone else's edits.
4. **Reset to committed data.json** discards local changes and reloads whatever's
   currently in the repo (or currently on the Sheet, if Live Sync is connected).
