/* Golf Trip Leaderboard — app logic
   Live data lives in a Google Sheet (via the Apps Script backend in
   google-sheet-backend/Code.gs) so every device sees the same thing
   automatically. localStorage is kept as a fast local cache and an
   offline fallback, not the source of truth. */

const LS_KEY = "golfTripState_v1";

/* Paste the URL you get after deploying google-sheet-backend/Code.gs as a
...
function setApiUrl(url) {
  localStorage.setItem("golfTripApiUrl", url || "");
}
...
/* Reads from Apps Script via JSONP (a <script src="..."> tag) instead of
   fetch(). This matters: browsers apply CORS checks to fetch()/XHR, and
   Apps Script Web App responses often don't include the header those
   checks require — even though the exact same URL works fine typed
   directly into the address bar (a plain page load isn't a CORS request).
   <script> tags load cross-origin content with no CORS check at all, which
   is exactly why JSONP exists as a technique. The Apps Script side (see
   Code.gs) wraps its JSON reply in "callbackName(...)" when asked to. */
function jsonpRequest(url, params, timeoutMs) {
  return new Promise((resolve, reject) => {
    const cbName = "goldTripJsonp" + (jsonpCounter++) + "_" + Date.now();
    const qs = Object.entries(params || {})
      .map(([k, v]) => encodeURIComponent(k) + "=" + encodeURIComponent(v))
      .concat(["callback=" + cbName, "t=" + Date.now()])
      .join("&");
...
async function fetchFromApi() {
  ...
  const body = await jsonpRequest(url, {});
  if (!body.ok) throw new Error(body.error || "api-error");
  return body.data;
}

/* Saves the whole current STATE to the Sheet. Fires the write as a normal
   POST (a "simple request" under CORS rules since it's sent as text/plain,
   so the browser sends it without a preflight regardless of what the
   response headers look like) — but since reading that POST's response
   body hits the same CORS wall as GET did, we don't trust it. Instead,
   after firing the write, we confirm it actually landed with a JSONP read
   (which is reliable) and compare it against what we tried to save. */
async function syncToApi() {
  ...
  try {
    fetch(url, {
      ...
      body: attempted
    }).catch(() => {}); // response is unreadable cross-origin; ignore, we confirm via JSONP below instead

    let confirmed = false;
    for (let attempt = 0; attempt < 4 && !confirmed; attempt++) {
      await new Promise(r => setTimeout(r, 900));
      try {
        const fresh = await jsonpRequest(url, {});
        if (fresh.ok && JSON.stringify(fresh.data) === attempted) confirmed = true;
      } catch (e) { /* try again */ }
    }
    if (!confirmed) throw new Error("save-not-confirmed");
    syncStatus = "saved"; lastSyncedAt = new Date();
  } catch (e) {
    syncStatus = "error";
  }
  renderSyncBadge();
...
async function loadState() {
  ...
  if (isApiConfigured()) {
    try {
      ...
      syncStatus = "offline";
      // fall through to cache/local file below
    }
  }
  if (cached) {
...
/* Pulls the latest from the Sheet and re-renders, but only if nobody is
   actively typing in a scorecard right now (so we don't yank an in-progress
   entry out from under someone). Used for both manual refresh and polling. */
async function refreshFromApi(showToastOnNoChange) {
  if (!isApiConfigured()) return;
  const active = document.activeElement;
  const isTyping = active && (active.classList.contains("sc-hole") || active.tagName === "SELECT" || active.tagName === "INPUT");
  if (isTyping) return;
  try {
    ...
    localStorage.setItem(LS_KEY, JSON.stringify(STATE));
    syncStatus = "saved"; lastSyncedAt = new Date();
    if (changed) { render(); toast("Updated with the latest scores"); }
    else { renderSyncBadge(); if (showToastOnNoChange) toast("Already up to date"); }
  } catch (e) {
    syncStatus = "error"; renderSyncBadge();
  }
}
...
function startPolling() {
  ...
  if (!isApiConfigured()) return;
  pollTimer = setInterval(() => refreshFromApi(false), 20000);
}

function saveState() {
  localStorage.setItem(LS_KEY, JSON.stringify(STATE));
  if (isApiConfigured()) syncToApi();
}
...
function syncStatusText() {
  if (!isApiConfigured()) return { label: "Local only \u2014 not connected to a shared sheet", cls: "sync-off" };
  const ago = lastSyncedAt ? Math.max(0, Math.round((Date.now() - lastSyncedAt.getTime()) / 1000)) : null;
  if (syncStatus === "saving") return { label: "Saving\u2026", cls: "sync-saving" };
  if (syncStatus === "error") return { label: "Sync failed \u2014 check connection, then tap Refresh", cls: "sync-error" };
  if (syncStatus === "offline") return { label: "Offline \u2014 showing last saved data", cls: "sync-error" };
  if (syncStatus === "saved") return { label: ago !== null ? `Synced ${ago < 5 ? "just now" : ago + "s ago"}` : "Synced", cls: "sync-ok" };
  return { label: "", cls: "sync-off" };
}
...
function renderSyncBadge() {
  ...
  el.innerHTML = `<span class="sync-dot"></span>${s.label}${isApiConfigured() ? ` <button class="sync-refresh-btn" data-action="manualRefresh" title="Refresh now">\u21bb</button>` : ""}`;
}
...
function renderAdmin() {
  return `
    <div class="card admin-section">
      <div class="card-title">Live Sync (Google Sheet)</div>
      <div class="note" style="margin-bottom:8px">
        ${isApiConfigured()
          ? "Connected \u2014 every save on this device goes straight to the shared sheet, and everyone else sees it automatically (no export/commit needed)."
          : "Not connected yet \u2014 saves currently only live in this browser. Follow SETUP_GOOGLE_SHEET.md once, then paste the Web App URL it gives you below."}
      </div>
      <div class="field-row">
        <div style="flex:1; min-width:260px"><label>Apps Script Web App URL</label><input type="text" id="apiUrlInput" value="${getApiUrl()}" placeholder="https://script.google.com/macros/s/XXXXX/exec" style="width:100%"></div>
      </div>
      <div class="io-buttons">
        <button class="brass" data-action="saveApiUrl">Connect</button>
        ${isApiConfigured() ? `<button class="ghost" data-action="testApiConnection">Test connection</button><button class="danger" data-action="disconnectApi">Disconnect</button>` : ""}
      </div>
    </div>
    ...
    <div class="card admin-section">
      ...
      <div class="note">
        ${isApiConfigured()
          ? "You're on Live Sync, so you don't need to do anything here day-to-day. These buttons are just for taking a manual backup, or restoring from one if something ever looks wrong."
          : "Not connected to Live Sync. Click <strong>Export data.json</strong>, then replace <code>data/data.json</code> in the repo and commit \u2014 anyone who loads the page fresh (no local edits) will see the update."}
      </div>
      <div class="io-buttons">
...
function handleAction(e) {
  ...
  const actions = {
    saveApiUrl() {
      const url = val("apiUrlInput").trim();
      if (!url) { toast("Paste the Web App URL first"); return; }
      setApiUrl(url);
      toast("Connecting\u2026 pulling latest data");
      loadState().then(() => { startPolling(); render(); });
    },
    testApiConnection() {
      toast("Checking connection\u2026");
      fetchFromApi().then(() => toast("Connected \u2014 sheet is reachable")).catch(() => toast("Could not reach the sheet \u2014 check the URL and deployment access"));
    },
    disconnectApi() {
      if (!confirm("Disconnect from the shared sheet? Saves will go back to being local-only on this device.")) return;
      setApiUrl("");
      if (pollTimer) clearInterval(pollTimer);
      syncStatus = "idle";
